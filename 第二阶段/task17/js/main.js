/* 数据格式演示
var aqiSourceData = {
  "北京": {
    "2016-01-01": 10,
    "2016-01-02": 10,
    "2016-01-03": 10,
    "2016-01-04": 10
  }
};
*/

// 以下两个函数用于随机模拟生成测试数据
function getDateStr(dat) {
  var y = dat.getFullYear();
  var m = dat.getMonth() + 1;
  m = m < 10 ? '0' + m : m;
  var d = dat.getDate();
  d = d < 10 ? '0' + d : d;
  return y + '-' + m + '-' + d;
}
function randomBuildData(seed) {
  var returnData = {};
  var dat = new Date("2016-01-01");
  var datStr = ''
  for (var i = 1; i < 92; i++) {
    datStr = getDateStr(dat);
    returnData[datStr] = Math.ceil(Math.random() * seed);
    dat.setDate(dat.getDate() + 1);
  }
  return returnData;
}

var aqiSourceData = {
  "北京": randomBuildData(500),
  "上海": randomBuildData(300),
  "广州": randomBuildData(200),
  "深圳": randomBuildData(100),
  "成都": randomBuildData(300),
  "西安": randomBuildData(500),
  "福州": randomBuildData(100),
  "厦门": randomBuildData(100),
  "沈阳": randomBuildData(500)
};

// 用于渲染图表的数据
var chartData = {};

// 记录当前页面的表单选项
var pageState = {
  nowSelectCity: -1,
  nowGraTime: "day"
}
function gethead(){
    switch(pageState.nowGraTime){
        case 'day':return '每日';
        case 'week':return '周平均';
        case 'month':return '月平均';
    }
}
/**
 * 渲染图表
 */
function renderChart() {
	/*a3.getElementsByClassName：表示指定类名的元素集合*/
	var aqiChartWrap=document.getElementsByClassName('aqi-chart-wrap')[0];
	var color='',text='';
    text='<h2 class="head">'+pageState.nowSelectCity+'市1月至3月'+gethead()+'空气质量报告</h2>';
	for(var item in chartData){
		/*a1.Math.random：随机一个大于等于0小于1的数*/
		/*a2.toString：把一个值转换为字符串，参数16为16进制表示字符串的值*/
		color='#'+('00000'+(Math.random()*0x1000000<<0).toString(16)).substr(-6); 
		// text+='<div title=\"'+item+':'+chartData[item]+'\" style=\"height:'+
		// 		chartData[item]+'px;background-color:'+color+';\"></div>';
		text+='<div style="height:'+chartData[item]+'px;background-color:'+color+';"></div>';
		text+='<p class="hint" style="height:'+chartData[item]+'px;">'+item+'<br/>[AQI]:'+chartData[item]+'</p>';

		
	}
	aqiChartWrap.innerHTML=text;
	
}

/**
 * 日、周、月的radio事件点击时的处理函数
 */
function graTimeChange() {
  // 确定是否选项发生了变化 
  	var span=document.getElementsByTagName('span');
  	for(var i=0;i<span.length;i++){
  		span[i].className="";
  	}
  	// previousSibling
  	this.previousSibling.className="selected";
  	console.log(this.previousSibling);
  	if(pageState.nowGraTime===this.value){
  		return;
  	}
  	pageState.nowGraTime=this.value;
  	
  	
  // 设置对应数据
  	initAqiChartData();
  // 调用图表渲染函数
  	renderChart();
}

/**
 * select发生变化时的处理函数
 */
function citySelectChange() {
  // 确定是否选项发生了变化 
  	if(pageState.nowSelectCity===this.value){
  		return;
  	}
  	pageState.nowSelectCity=this.value
  // 设置对应数据
  	initAqiChartData();
  // 调用图表渲染函数
  	renderChart();
}

/**
 * 初始化日、周、月的radio事件，当点击时，调用函数graTimeChange
 */
function initGraTimeForm() {	
	var formGraTime=document.getElementById('form-gra-time');
	var pageRadio=formGraTime.getElementsByTagName('input');
	for(var i=0;i<pageRadio.length;i++){
		pageRadio[i].addEventListener('click',graTimeChange);
	}
}

/**
 * 初始化城市Select下拉选择框中的选项
 */
function initCitySelector() {
  // 读取aqiSourceData中的城市，然后设置id为city-select的下拉列表中的选项
  	var citySelect=document.getElementById('city-select');
  	var cityList='';
  	for(var i in aqiSourceData){
  		if(pageState.nowSelectCity===-1){
  			pageState.nowSelectCity=i;
  		}
  		cityList+='<option>'+i+'</option>'
  	}
  	citySelect.innerHTML=cityList;
  // 给select设置事件，当选项发生变化时调用函数citySelectChange
  	citySelect.addEventListener('change',citySelectChange);	

}

/**
 * 初始化图表需要的数据格式
 */
function initAqiChartData() {
  // 将原始的源数据处理成图表需要的数据格式
  // 处理好的数据存到 chartData 中 	
  	var nowCityData=aqiSourceData[pageState.nowSelectCity];/*获取对应城市的数据*/

  	if(pageState.nowGraTime==='day'){/*当前选择为day，不做处理，直接赋值*/
  		chartData=nowCityData;
  	}
  	
  	if(pageState.nowGraTime==='week'){/*当前选择为week时，*/
  		chartData={};
  		var countSum=0,daySum=0,week=0;
  		for(var i in nowCityData){
  			countSum=countSum+nowCityData[i];
  			daySum++;
  			if((new Date(i)).getDay()===6){
  				week++;
  				chartData['第'+week+'周']=Math.floor(countSum/daySum);
  				countSum=0;
  				daySum=0;
  			}
  		}
  		if(daySum!=0){
  			week++;
  			chartData['第'+week+'周']=Math.floor(countSum/daySum);
  		}
  	}
  	if(pageState.nowGraTime==='month'){
  		chartData={};
  		var countSum=0,daySum=0,month=0;
  		for(var i in nowCityData){
  			countSum+=nowCityData[i];
  			daySum++;
  			if((new Date(i)).getMonth()!=month){
  				month++;
  				chartData['第'+month+'月']=Math.floor(countSum/daySum);
  				countSum=0;
  				daySum=0;
  			}
  		}
  		if(daySum!=0){
  			chartData['第'+month+'月']=Math.floor(countSum/daySum);
  		}
  	}
    renderChart();
}

/**
 * 初始化函数
 */
function init() {
  initGraTimeForm()
  initCitySelector();
  initAqiChartData();
  
}

init();
